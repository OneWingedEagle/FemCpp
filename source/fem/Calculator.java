...
public Vect[] gradNPrism(Mat jac,Vect localCo){
	...
	public  Vect[] rotNe(Mat jac, Vect localCo){
		...
		if(this.elCode==2) return rotNePrism(jac,localCo);
...
*/
	...
	Vect[] gradNPrismx(Model model,int ie,double h){

		Node[] vertexNode=model.elementNodes(ie);

		Vect z1=vertexNode[0].getCoord().v2();
		Vect z2=vertexNode[1].getCoord().v2();
		Vect z3=vertexNode[2].getCoord().v2();

		Vect c=z1.add(z2).add(z3).times(1.0/3);
		...
		double H=vertexNode[0].getCoord(2)-vertexNode[3].getCoord(2);
		double c1=(1+h)/2;
		double c2=(1-h)/2;
		double g=2/H;
		...
		gradN[0]=new Vect(z2.el[1]-z3.el[1],z3.el[0]-z2.el[0]).times(c1);
		gradN[1]=new Vect(z3.el[1]-z1.el[1],z1.el[0]-z3.el[0]).times(c1);
		gradN[2]=new Vect(z1.el[1]-z2.el[1],z2.el[0]-z1.el[0]).times(c1);
		gradN[3]=new Vect(z2.el[1]-z3.el[1],z3.el[0]-z2.el[0]).times(c2);
		gradN[4]=new Vect(z3.el[1]-z1.el[1],z1.el[0]-z3.el[0]).times(c2);
		gradN[5]=new Vect(z1.el[1]-z2.el[1],z2.el[0]-z1.el[0]).times(c2);



		for(int i=0;i<3;i++)
			gradN[i]=new Vect(gradN[i].el[0],gradN[i].el[1],N[i]*g);
		for(int i=3;i<6;i++)
			gradN[i]=new Vect(gradN[i].el[0],gradN[i].el[1],-N[i-3]*g);
		/*	for(int i=0;i<6;i++)
			if(i%2==0)
			gradN[i].el[2]*=-.5;
			else
...
/*
	...
	public Vect getBAt(Model model,int i,Vect lc){
		...
		else if(this.elCode==3) return new Vect(dim);
