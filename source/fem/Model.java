...
public class Model{
	...
	public void setSeepMat(boolean massNeeded){
		seepMat.setSeepMat(this,massNeeded);
	}
...
Vect Ci=this.Cs.scale(this.bT);
	...
	public void  setSeepage(){
		...
		if(this.numberOfUnknownT!=0){

			femSolver.solveSeepage(this);	
...
/*			SpMat P=Ks.timesNew(1e-9);
	...
	public void setTorque(Vect origin, double r1,double r2, int mode){
		...
		for(int i=1;i<=this.numberOfNodes;i++)
		{
			...
			double rn=this.node[i].getCoord().v2().norm();
...
//Je.hshow();
	...
	public void setJfJ(int ir,double t){
		...
		double ju=0;
/*		if(this.numberOfRegions==18){
			double rds=50/5.5e-4;
			 ju=0;

			 int nc1=1,nc2=6,nc3=11;
		if(ir<nc1+5){
			ju=this.ia.getI(t)*rds;
			double jud=ju/2;
			if(ir==nc1) J=new Vect(0,0,ju);
			else if(ir==nc1+1) J=new Vect(jud,0,jud);
			else if(ir==nc1+2) J=new Vect(ju,0,0);
			else if(ir==nc1+3) J=new Vect(jud,0,-jud);
			else if(ir==nc1+4) J=new Vect(0,0,-ju);
	
		}
	
		else if(ir<nc2+5){
			ju=this.ib.getI(t)*rds;
			double jud=ju/2;
			if(ir==nc2) J=new Vect(0,0,ju);
			else if(ir==nc2+1) J=new Vect(jud,0,jud);
			else if(ir==nc2+2) J=new Vect(ju,0,0);
			else if(ir==nc2+3) J=new Vect(jud,0,-jud);
			else if(ir==nc2+4) J=new Vect(0,0,-ju);
	
		}
		else if(ir<nc3+5){
			ju=this.ic.getI(t)*rds;
			double jud=ju/2;
			if(ir==nc3) J=new Vect(0,0,ju);
			else if(ir==nc3+1) J=new Vect(jud,0,jud);
			else if(ir==nc3+2) J=new Vect(ju,0,0);
			else if(ir==nc3+3) J=new Vect(jud,0,-jud);
			else if(ir==nc3+4) J=new Vect(0,0,-ju);
	...
	//	kf=.2*util.triangWave(this.region[ir].freq*(t+this.region[ir].phase0/2/PI));
...
if(this.numberOfRegions>8){
	...
	/*			if(ir==9 ) curr=cos(2*PI*100*t);
				else if(ir==10 ) curr=-cos(2*PI*100*t);
				else if(ir==11 ) curr=cos(2*PI*100*t-2*PI/3);
				else if(ir==12 ) curr=-cos(2*PI*100*t-2*PI/3);
				else if(ir==13 ) curr=cos(2*PI*100*t+2*PI/3);
				else if(ir==14 ) curr=-cos(2*PI*100*t+2*PI/3);
				
				curr*=3*1.4142;*/
		
			}
			//********************************************
	/*		else if(this.numberOfRegions==4){
				
				curr=0;

			if(ir==1 ) curr=this.ia.getI(t);
			else if(ir==2 ) curr=-this.ia.getI(t);
	...
	public void setJ3phaseStrandedCircuit(double t){
		...
		for(int j=0;j<3;j++){
			...
			double vt=util.triangWave(fc*t);
...
/*		int ir=this.unCurRegNumb[0];
	...
	public void resetMotor(){
		...
		int L=0;
		if(commonNodes!=null)
			L=commonNodes[0].length;
		for(int i=0;i<L;i++){
			int nn=commonNodes[0][i];
			node[nn].setMap(commonNodes[1][i]);
			node[nn].common=true;
		}
