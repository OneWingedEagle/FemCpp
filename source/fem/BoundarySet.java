...
public class BoundarySet {
	...
	public void mapCommonNodes(Model model){
		...
		int[][] commonNodeNumbSorted=getCommonNodeSorted(model);

		model.commonNodes=commonNodeNumbSorted;

		if( !model.hasTwoNodeNumb) return;
		int L=0;
		if(commonNodeNumbSorted!=null)
			L=commonNodeNumbSorted[0].length;

		for(int i=0;i<L;i++){
			int n1=commonNodeNumbSorted[0][i];
			int n2=commonNodeNumbSorted[1][i];
