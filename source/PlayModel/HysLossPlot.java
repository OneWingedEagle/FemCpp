...
public static void main(String[] args){
	...
	HysLossPlot pg=new HysLossPlot();
...
public void loadLoss(){
	...
	String file="C:\\Works\\PlayModel\\hys_loss";
...
for(int k=0;k<sp.length;k++)
	if(sp[k].startsWith("Alt"))
		nAngs++;

String[] name=new String[nAngs+2];

int Nmax=2000;

Mat[] BL1=new Mat[nAngs+2];
BL=new Mat[nAngs+2];
for( int p=0;p<nAngs+2;p++)
	BL1[p]=new Mat(Nmax,2);
...
int jx=0;
for(int k=0;k<sp.length;k++)
	if(sp[k].startsWith("deg")){
		name[jx++]="Alt. loss "+sp[k-1]+" .deg";
	}
name[nAngs]="Rot. loss ";
name[nAngs+1]="Modif. Rot. loss ";
...
for( int p=0;p<nAngs+2;p++){
	BL1[p].el[ix][0]=data[0];
	BL1[p].el[ix][1]=data[p+1];
}
...
for( int p=0;p<nAngs+2;p++){
	...
	for(int i=0;i<ix;i++)
		BL[p].el[i]=BL1[p].el[i];
			...
			util.plotBunch(name,BL);

		}
		
			catch(IOException e){System.err.println("Error occured in loading hys_loss file.");
