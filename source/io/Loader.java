...
public static void main2(String[] args){
		...
		for(int i=0;i<0;i++){
			...
			String file1=System.getProperty("user.dir") + "\\fluxp1\\flux"+df.format(sf1)+".txt";
	...
	public void setDataSeep(Model model,BufferedReader br){
		...
		try {
			...
			line=br.readLine();
			readAndSetRegDataSeep(model,ir,line);
			}
			...
			String sp[]=line.split(regex);
			for(int k=0;k<sp.length;k++){
				model.seepBC[j][k]=sp[k];
			}
			}
			
			
			model.setElementsParamSeep();
...
/*			for(int i=1;i<=model.numberOfNodes;i++){
	...
	public void loadNodalUx(){
		String mf="C:\\JavaProjects\\prj6-inputVolt\\motWithShaft3D.txt";
		...
		for(int j=90;j<L;j++){
			...
			String uuFile=System.getProperty("user.dir")+"\\disp6ABothFull-0-90D\\disp"+(j+2)+".5.txt";
			// uuFile=System.getProperty("user.dir")+"\\disp6AMagFull-0-90D\\disp"+(j+2)+".5.txt";
			// util.pr(uuFile);
			model.loadNodalField(uuFile, -1);
			
			 // uuFile=System.getProperty("user.dir")+"\\disp6ABothFull-0-90DLight\\disp"+(j+2)+".5.txt";
			 // uuFile=System.getProperty("user.dir")+"\\disp6AMagFull-0-90DLight\\disp"+(j+2)+".5.txt";
			  model.writeNodalField(uuFile, -1);
		}
	...
	private int[] getBCdata(String line){
		...
		if(sp[sp.length-1].equalsIgnoreCase("s")) pair=-2;
